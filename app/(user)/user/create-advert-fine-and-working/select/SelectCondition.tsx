...
export const SelectCondition = ({ condition, onChange }: SelectOptionTypes) => 
{
    ...
    useEffect(() => 
    {
       setPlaceHolder("Select Condition")
    }, [])
    ...
    {
       const OPTIONS = condition?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [condition, search])
            ...
            >
               ...
               >
                  ...
                  >                     
                     ...
                     {
                        condition?.length > 0 && search?.length === 0 && condition?.map((condition: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 ...
                                 className={
                                    ...
                                    ${
                                       condition?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                       "bg-green-600 text-white"
                                    }
                                 `}
                                 onClick={
                                    () => {
                                       onChange(condition?.name, condition?.id)
                                       setOpen(false)
                                       setSearch(condition?.name)
                                       setHoldData(condition?.name)                     
                                       hasItem(condition?.name)
                                    }
                                 }
                                 onFocus={
                                    () => {
                     ...
                     {
                        conditionSearch?.length > 0 && search?.length > 0 && conditionSearch?.map((condition: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 // key={index} ref={el => itemRef.current[condition?.id] = el}
                                 key={index} ref={itemRef}
                                 className={
                                    `w-full hover:bg-blue-600 hover:border-2 hover:border-green-200 
                                    hover:text-white cursor-pointer focus:border-indigo-200 

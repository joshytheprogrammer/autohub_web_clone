...
type SelectOptionTypes = 
{
    ...
    selectedTransmissionName: string
    onChange: (name: string, x: number) => void
}

export const SelectTransmission = ({ transmissions, selectedTransmission, selectedTransmissionName, onChange }: SelectOptionTypes) => 
{
   ...
   const [changeView, setChangeView]= useState<any>(-1)
    ...
    useEffect(() => 
    {
       setPlaceHolder("Select Transmission")
    }, [])
    ...
    useEffect(() => 
    {
       setTimeout(() => 
      {          
         if(selectedTransmission != -1 && transmissions?.length > 0)
         {
            setPlaceHolder("Select colour")         
         }
         if(selectedTransmissionName != "" || selectedTransmissionName != null)
         {
            setSearch(selectedTransmissionName)
            setHoldData(selectedTransmissionName)
         } else {         
            setSearch("")
            setHoldData("")
         }
      }, 500)       
    }, [transmissions, selectedTransmissionName])

    const transmissionSearch: any  = useMemo(() => 
    {
       const OPTIONS = transmissions?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [transmissions, search])
            ...
            >
               ...
               >
                  ...
                  >                     
                     {  search?.length === 0 &&                    
                        <li 
                           className={`${isOpen} text-gray-700 cursor-pointer hover:bg-green-600 hover:border-2 hover:border-green-200 hover:text-white font-bold`}
                           onClick={
                              () => {
                                 setIsSelected(false)
                                 setSearch("")
                     ...
                     {
                        transmissions?.length > 0 && search?.length === 0 && transmissions?.map((transmission: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 // key={index} ref={el => itemRef.current[category?.id] = el}
                                 ...
                                 className={
                                    ...
                                    ${
                                       transmission?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                       "bg-green-600 text-white"
                                    }
                                 `}
                                 onClick={
                                    () => {
                                       onChange(transmission?.name, transmission?.id)
                                       setOpen(false)
                                       setSearch(transmission?.name)
                                       setHoldData(transmission?.name)                     
                                       hasItem(transmission?.name)
                                       advertState.setTransmission(transmission?.id)
                                       advertState.setTransmissionName(transmission?.name)
                                       setOpen(false)
                                    }
                                 }
                                 onFocus={
                     ...
                     {
                        transmissionSearch?.length > 0 && search?.length > 0 && transmissionSearch?.map((transmission: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 // key={index} ref={el => itemRef.current[transmission?.id] = el}
                                 key={index} ref={itemRef}
                                 className={
                                    `w-full hover:bg-blue-600 hover:border-2 hover:border-green-200 
                                    hover:text-white cursor-pointer focus:border-indigo-200 
                     ...
                     {
                        transmissionSearch?.length === 0 && <div 

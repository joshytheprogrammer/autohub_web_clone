...
export const SelectCondition = ({ conditions, selectedCondition, selectedConditionName, onChange }: SelectOptionTypes) => 
{
    ...
    useEffect(() => 
    {
       setTimeout(() =>
       {  
         if(advertState.getConditionName()  === "")
         {
           setPlaceHolder("Select conditions")
           setHoldData("")
           setSearch("")
         } else {          
           setSearch(advertState.getConditionName())
           setHoldData(advertState.getConditionName()) 
         }
       }, 100)
    }, [])
    ...
    useEffect(() => 
    {
       setTimeout(() => 
      {          
         if(selectedCondition != -1 && conditions?.length > 0)
         {
            setPlaceHolder("Select colour")         
         }
         if(selectedConditionName != "" || selectedConditionName != null)
         {
            setSearch(selectedConditionName)
            setHoldData(selectedConditionName)
         } else {         
            setSearch("")
            setHoldData("")
         }
      }, 500)       
    }, [conditions, selectedConditionName])

    const conditionSearch: any  = useMemo(() => 
    {
       const OPTIONS = conditions?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [conditions, search])
            ...
            >
               ...
               >
                  ...
                  >                     
                     ...
                     {
                        conditions?.length > 0 && search?.length === 0 && conditions?.map((conditions: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 ...
                                 className={
                                    ...
                                    ${
                                       conditions?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                       "bg-green-600 text-white"
                                    }
                                 `}
                                 onClick={
                                    () => {
                                       onChange(conditions?.name, conditions?.id)
                                       setSearch(conditions?.name)
                                       setHoldData(conditions?.name)                     
                                       hasItem(conditions?.name)
                                       advertState.setCondition(conditions?.id)
                                       advertState.setConditionName(conditions?.name)
                                       setOpen(false)
                                    }
                                 }
                                 onFocus={
                     ...
                     {
                        conditionSearch?.length > 0 && search?.length > 0 && conditionSearch?.map((conditions: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 // key={index} ref={el => itemRef.current[conditions?.id] = el}
                                 key={index} ref={itemRef}
                                 className={
                                    `w-full hover:bg-blue-600 hover:border-2 hover:border-green-200 
                                    hover:text-white cursor-pointer focus:border-indigo-200 

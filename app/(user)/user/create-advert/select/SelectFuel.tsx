...
export const SelectFuel = ({ fuel, selectedFuelName, onChange }: SelectOptionTypes) => 
{
    ...
    useEffect(() => 
    {
       setPlaceHolder("Select Fuel")
       setTimeout(() =>
       {         
         setSearch(selectedFuelName)
         setHoldData(selectedFuelName)
       }, 100)
    }, [])
    ...
    const fuelSearch: any  = useMemo(() => 
    {
       const OPTIONS = fuel?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [fuel, search])
            ...
            >
               ...
               >
                  ...
                  >                     
                     ...
                     {
                        fuel?.length > 0 && search?.length === 0 && fuel?.map((fuel: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 ...
                                 className={
                                    ...
                                    ${
                                       fuel?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                       "bg-green-600 text-white"
                                    }
                                 `}
                                 onClick={
                                    () => {
                                       onChange(fuel?.name, fuel?.id)
                                       setSearch(fuel?.name)
                                       setHoldData(fuel?.name)                     
                                       hasItem(fuel?.name)
                                       advertState.setFuel(fuel?.id)
                                       advertState.setFuelName(fuel?.name)
                                       setOpen(false)
                                    }
                                 }
                                 onFocus={
                     ...
                     {
                        fuelSearch?.length > 0 && search?.length > 0 && fuelSearch?.map((fuel: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 // key={index} ref={el => itemRef.current[fuel?.id] = el}
                                 key={index} ref={itemRef}
                                 className={
                                    `w-full hover:bg-blue-600 hover:border-2 hover:border-green-200 
                                    hover:text-white cursor-pointer focus:border-indigo-200 

...
export const SelectSerie = ({ selectedManufacturer, selectedModel, selectedGeneration, series = [], selectedSerieName, onChange }: SelectOptionTypes) => 
{
    ...
    useEffect(() => 
    {
       if((selectedManufacturer === -1 || selectedModel === -1 || selectedGeneration === -1) && series?.length === 0)
       {
       ...
       if((selectedManufacturer != -1 || selectedModel != -1 || selectedGeneration != -1) && series?.length === 0)
       {
          setPlaceHolder("No Series")
       }
       if((selectedManufacturer === -1 || selectedModel === -1 || selectedGeneration === -1) && series?.length > 0)
       {
          setTimeout(() =>
          {  
            if(advertState.getSerieName()  === "")
            {
               setPlaceHolder("First select generation")
               setHoldData("")
               setSearch("")
               ...
               {console.log("YYYeeeeeeeeeeeaaaaah")
                  ...
                  setSearch(advertState.getSerieName())
                  setHoldData(advertState.getSerieName()) 
               }
            }
          }, 100)
       }
    ...
    useEffect(() => 
    {
       setPlaceHolder("First select a generation")
       setSearch("")
       setHoldData("") 
    }, [selectedManufacturer, selectedModel, selectedGeneration])
    ...
    const serieSearch: any  = useMemo(() => 
    {
       const others = series?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	

       const popular = series?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)
    ...
    {
       const OPTIONS = series?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [series, search])
   ...
   //  useMemo(() => setSearch(value), [value])
    ...
    return (
         <> 
            {/* ({selectedManufacturer}) - ({selectedModel}) - ({selectedGeneration}) -  ({series?.length}) */}
            <div 
            ...
            >
               ...
               >
                  <input 
                     ...
                     autoComplete="false"
                     disabled={(selectedManufacturer === -1 || selectedModel === -1 || selectedGeneration === -1) ? true : false}
                     onChange={
                        (e: any) => {
                  ...
                  >
                     {  search?.length === 0 && (selectedManufacturer != -1 || selectedModel != -1 || selectedGeneration != -1) && series?.length != 0 &&                    
                        <li 
                     ...
                     {
                        series?.length > 0 && search?.length === 0 && (selectedManufacturer !- -1 || selectedModel !- -1 || selectedGeneration !- -1) && series?.map((serie: { id: number, name: string }, index: number) => 
                        {
                           return (
                                 <>
                                    <li 
                                       ...
                                       className={
                                          ...
                                          duration-200 ease-in-out overflow-auto ${isOpen} z-0
                                          ${
                                             serie?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                             "bg-green-600 text-white"
                                          }
                                       `}
                                       onClick={
                                          () => {
                                             onChange(serie?.name, serie?.id)
                                             setOpen(false)
                                             setSearch(serie?.name)
                                             setHoldData(serie?.name)                     
                                             hasItem(serie?.name)
                                             advertState.setSerie(serie?.id)
                                             advertState.setSerieName(serie?.name)
                                             advertState.setTrims([])
                                             advertState.setTrim(-1)
                                             advertState.setTrimName("")
                                             setOpen(false)
                     ...
                     {
                        serieSearch?.length > 0 && search?.length > 0 && (selectedManufacturer != -1 || selectedModel != -1 || selectedGeneration != -1) && serieSearch?.map((serie: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 key={index} ref={itemRef}
                     ...
                     {
                        serieSearch?.length === 0 && (selectedManufacturer === -1 || selectedModel === -1 || selectedGeneration === -1) && series?.length != 0 && <div 

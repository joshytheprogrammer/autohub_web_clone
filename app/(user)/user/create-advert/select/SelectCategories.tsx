...
export const SelectCategories = ({ categories, selectedCategoryName, onChange }: SelectOptionTypes) => 
{
    ...
    useEffect(() => 
    {
       ...
       {         
         ...
         setHoldData(selectedCategoryName)
       }, 100)
    }, [])
    ...
    const categorySearch: any  = useMemo(() => 
    {
       const OPTIONS = categories?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [categories, search])
            ...
            >
               ...
               >
                  ...
                  >                     
                     ...
                     {
                        categories?.length > 0 && search?.length === 0 && categories?.map((category: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 ...
                                 className={
                                    ...
                                    ${
                                       category?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                 ...
                                 onClick={
                                    () => {
                                       ...
                                       setHoldData(category?.name)                     
                                       hasItem(category?.name)
                                       advertState.setCategory(category?.id)
                                       advertState.setCategoryName(category?.name)
                                       setOpen(false)
                                    }
                                 }
                                 onFocus={
                     ...
                     {
                        categorySearch?.length > 0 && search?.length > 0 && categorySearch?.map((category: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 // key={index} ref={el => itemRef.current[category?.id] = el}

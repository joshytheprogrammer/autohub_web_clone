...
export const SelectYear = ({ selectedManufacturer, selectedModel, selectedYear, selectedYearName, years = [], onChange }: SelectOptionTypes) => 
{
    ...
    useEffect(() => 
    {
       if(selectedManufacturer === -1 || selectedModel === -1)
       {
          ...
          setTimeout(() =>
          {  
            if(advertState.getYearName()  === "")
            {
               setPlaceHolder("First select a model")
               setHoldData("")
               setSearch("")
            } else {          
               if(Number(selectedYear) === -1)
               {console.log("YYYeeeeeeeeeeeaaaaah")
                  ...
                  setSearch(advertState.getYearName())
                  setHoldData(advertState.getYearName()) 
               }
            }
          }, 100)
       }
    ...
    const yearSearch: any  = useMemo(() => 
    {
       const others = years?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	

       const popular = years?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)
    ...
    {
       const OPTIONS = years?.filter((x: any) => x?.name.toString().toLocaleLowerCase().indexOf(search.toString().toLowerCase()) !== -1)	
       return OPTIONS
    }, [years, search])
            ...
            >
               ...
               >
                  <input 
                     ...
                     autoComplete="false"
                     disabled={(selectedManufacturer === -1 || selectedModel === -1) ? true : false}
                     onChange={
                        (e: any) => {
                  ...
                  >
                     {  search?.length === 0 && (selectedManufacturer != -1 || selectedModel != -1) && years?.length != 0 &&                    
                        <li 
                     ...
                     {
                        years?.length > 0 && search?.length === 0 && (selectedManufacturer != -1 || selectedModel != -1) && years?.map((year: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 ...
                                 className={
                                    ...
                                    ${
                                       year?.name.toString() === holdData &&
                                       "bg-green-600 text-white"
                                    }
                                 `}
                                 onClick={
                                    () => {
                                       onChange(year?.name, year?.id)
                                       setSearch(year?.name)
                                       setHoldData(year?.name)                     
                                       hasItem(year?.name)
                                       advertState.setYear(Number(year?.id))
                                       advertState.setYearName(year?.name)
                                       setOpen(false)
                                    }
                                 }
                                 onFocus={
                           ...
                           return (
                              <li 
                                 ...
                                 className={
                                    ...
                                    duration-200 ease-in-out overflow-auto ${isOpen}
                                    ${
                                       year?.name?.toLowerCase() === holdData?.toLowerCase() &&
                                       "bg-green-600 text-white"
                                    }
                                 `}
                                 onClick={
                     ...
                     {
                        yearSearch?.length > 0 && search?.length > 0 && (selectedManufacturer != -1 || selectedModel != -1) && yearSearch?.map((year: { id: number, name: string }, index: number) => 
                        {
                           return (
                              <li 
                                 key={index} ref={itemRef}
                     ...
                     {
                        yearSearch?.length === 0 && (selectedManufacturer === -1 || selectedModel === -1) && (years?.length != 0 || years?.length != 0) && <div 

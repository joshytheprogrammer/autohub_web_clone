  ...
  return (
      <>     
        ...
        { !isLoading && (data?.plus?.payment_status === "paid") && (data?.plus?.access === "granted") &&
            <>
              <div 
                className="grid grid-cols-12 gap-3 border border-solid border-green-900 col-span-12 p-5 md:p-3 rounded-lg"
              >                    
                {
                  !isLoading && data && 
                      <>
              ...
              {
                !isLoading && (data?.data?.objective.length === 0) && <>
                    <div 
              ...
              {
                !isLoading && data?.data?.objective && (data?.data?.objective?.length > 0) && <> 
                   <div 
                   ...
                   <div 
                    className="font-bold col-span-12 mt-3 mb-3 uppercase text-blue-600"
                  >
                    Objective
                  </div>
                 </> 
              ...
              
              {
                !isLoading && (Number(data?.data?.exam_objective) === 0) && 
                   <div 
                      className="flex justify-between mb-5 p-2 bg-white md:col-span-6 col-span-6 text-black font-bold text-sm ring-2 ring-blue-100 rounded-lg space-between px-3 border border-solid border-green-900"
                    >
                      <span className="px-3 text-md uppercase text-blue-800">Objective</span>
                      <span className="hidden md:block px-3"> {data?.data?.exam_objective} </span>
                   </div> 
              }
              {
              ...
              {/* theory  */}
              {
                !isLoading && (Number(data?.data?.exam_theory) === 0) && 
                  ...
                  >
                    ...
                    <span className="hidden md:block px-3"> {data?.data?.exam_theory} </span>
                  </div> 
              }
              {
                !isLoading && (data?.data?.exam_theory === "Not Yet Marked") && 
                  <div 
                    className="flex justify-between mb-5 p-2 bg-white md:col-span-6 col-span-6 text-black font-bold text-sm ring-2 ring-green-100 rounded-lg space-between px-3 border border-solid border-green-900"
                  >
              ...
              }
              {
                !isLoading && (Number(data?.data?.exam_theory) > 0)  && (data?.data?.exam_theory != "not-taken") && 
                  <div 
                    className="flex justify-between mb-5 p-2 bg-white md:col-span-6 col-span-6 text-black font-bold text-sm ring-2 ring-blue-100 rounded-lg space-between px-3 border border-solid border-blue-900"
                  >
                    <span className="px-3 text-md uppercase text-green-800">Theory</span>
                    <span className="hidden md:block px-3"> { data?.data?.exam_theory } </span>
                  </div> 

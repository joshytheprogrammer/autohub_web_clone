...
import { GetUsers } from '../../../api/admin/market/users'
...
export default function Users() 
{
    ...
    const { data, isLoading, refetch, isRefetching } = useQuery({ queryKey: [`get-all-product`, currentPage, perPage, searchQuery, token], queryFn: () => GetUsers(currentPage, perPage, searchQuery, token)})
    ...
    const users = useMemo<ColumnDef<AllProducts>[]>(
          () => [            
            {
                  header: 'Picture',
    ...
    return (
            ...
            >                 
                ...
                {  !isLoading && (data?.data?.users?.user.length === 0) && <>
                        <div 
                            className="flex md:d-flex xl:flex-row h-[400px] justify-center items-center mt-20"
                        >
                            <div 
                            ...
                            >
                                <div className="w-full text-center text-lg">No User Yet</div>
                            </div>
                        </div>
                    </>
                }
                  
                {  !isLoading && (data?.data?.users?.user.length === 0) && searchQuery && <>
                        <div 
                            className="flex md:d-flex xl:flex-row h-[400px] justify-center items-center mt-20"
                        >
                            <div 
                        ...
                        <div 
                            ...
                            >                    
                                <div 
                                        className='px-5'
                                >                              
                                    <Table data={data?.data?.users?.user} 
                                            columns={users} 
                ...
                { 
                        !isLoading && !isRefetching && (data?.data?.users?.user.length > 0) && 
                                ...
                                noOfPages={data?.data?.users?.noOfPages} 
                                hasNextPage={data?.data?.users?.hasNextPage} 
                                hasPreviousPage={data?.data?.users?.hasPreviousPage} 
                            />    
                }
                { 
                    !isLoading && isRefetching && (data?.data?.users?.user.length > 0) && 

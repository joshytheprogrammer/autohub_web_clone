...
import { GetAffiliates } from '../../../api/admin/market/users'
...
export default function Affiliates() 
{
    ...
    const pages = [1, 50, 100, 200, 350, 500, 1000]
    const [currentPage, setCurrentPage] = useState<number>(1)  
    const [perPage, setPerPage] = useState<number>(pages[0])  
    const [searchQuery, setSearchQuery] = useState("")

    const { data, isLoading, refetch, isRefetching } = useQuery({ queryKey: [`get-all-affiliate`, currentPage, perPage, searchQuery, token], queryFn: () => GetAffiliates(currentPage, perPage, searchQuery, token)})
    ...
    const Affiliate = useMemo<ColumnDef<AllAffiliate>[]>(
          () => [            
            {
                  header: 'Picture',
    ...
    return (
            ...
            >                 
                ...
                {  !isLoading && (data?.data?.affiliates?.affiliate.length === 0) && <>
                        <div 
                            className="flex md:d-flex xl:flex-row h-[400px] justify-center items-center mt-20"
                        >
                            <div 
                ...
                {  !isLoading && (data?.data?.affiliates?.affiliate.length === 0) && searchQuery && <>
                        <div 
                            className="flex md:d-flex xl:flex-row h-[400px] justify-center items-center mt-20"
                        >
                            <div 
                        ...
                        <div 
                            ...
                            >                    
                                <div 
                                        className='px-5'
                                >                              
                                    <Table data={data?.data?.affiliates?.affiliate} 
                                            columns={Affiliate} 
                ...
                { 
                        !isLoading && !isRefetching && (data?.data?.affiliates?.affiliate.length > 0) && 
                                ...
                                noOfPages={data?.data?.affiliates?.noOfPages} 
                                hasNextPage={data?.data?.affiliates?.hasNextPage} 
                                hasPreviousPage={data?.data?.affiliates?.hasPreviousPage} 
                            />    
                }
                { 
                    !isLoading && isRefetching && (data?.data?.affiliates?.affiliate.length > 0) && 

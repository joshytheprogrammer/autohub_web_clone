...
import { GetDealers } from '../../../api/admin/market/users'
...
export default function Blogs() 
{
    ...
    const { data, isLoading, refetch, isRefetching } = useQuery({ queryKey: [`get-all-product`, currentPage, perPage, searchQuery, token], queryFn: () => GetDealers(currentPage, perPage, searchQuery, token)})
    ...
    const Dealer = useMemo<ColumnDef<AllDealers>[]>(
          () => [            
            {
                  header: 'Picture',
    ...
    return (
            ...
            >                 
                ...
                }
                {
                  ...
                  {  !isLoading && (data?.data?.dealers?.dealer.length === 0) && <>
                        <div 
                            className="flex md:d-flex xl:flex-row h-[400px] justify-center items-center mt-20"
                        >
                            <div 
                            ...
                            >
                                <div className="w-full text-center text-lg">No Dealer Yet</div>
                            </div>
                        </div>
                    </>
                }
                {  !isLoading && (data?.data?.dealers?.dealer.length === 0) && searchQuery && <>
                    <div 
                        className="flex md:d-flex xl:flex-row h-[400px] justify-center items-center mt-20"
                    >
                        <div 
                        ...
                        <div 
                            ...
                            >                    
                                <div 
                                        className='px-5'
                                >                              
                                    <Table data={data?.data?.dealers?.dealer} 
                                            columns={Dealer} 
                ...
                { 
                        !isLoading && !isRefetching && (data?.data?.dealers?.dealer.length > 0) && 
                                ...
                                noOfPages={data?.data?.dealers?.noOfPages} 
                                hasNextPage={data?.data?.dealers?.hasNextPage} 
                                hasPreviousPage={data?.data?.dealers?.hasPreviousPage} 
                            />    
                }
                { 
                    !isLoading && isRefetching && (data?.data?.dealers?.dealer.length > 0) && 

...
type EditSelectStateProps = 
{
    ...
    stateOption: string
    edit: boolean
    onClick: (stateId: number) => void
}


const EditLGAState = ({ placeholder, states, countryId, countryName, selectedStateName, stateOption, edit, onClick }: EditSelectStateProps) => 
{
  const [inputValue, setInputValue] = useState("")
  const [selected, setSelected] = useState("")
  const [cryId, setCountryId] = useState<number>(-1)
  const [open, setOpen] = useState<boolean>(false)
  const [choosen, setChoosen] = useState<string>(stateOption) 
  
  useEffect(() => 
  {
    setSelected(selectedStateName)
            ...
            { states && (states.length > 0) && (countryId != -1) &&
                <>
                    ...
                    >
                        ...
                        {
                            states?.map((x: { tb_id: number, name: string }, index: number) => (
                                <li
                                    key={index}
                                    className={`p-2 text-md hover:bg-sky-600 border-2 border-gray-200 hover:border-2 hover:border-green-200 hover:text-white cursor-pointer py-3
                                    ${

...
type EditSearchTrimProps = 
{
    placeholder: string
    selectedTrim: string
    onClick: (trimId: number, trimName: string) => void
    trims: any
    modelId: number
...
const EditSearchTrim = ({ placeholder, selectedManufacturerName, modelId, selectedModelName, selectedTrim, trimOption, trims, onClick }: EditSearchTrimProps) => 
{
  ...
  return (
        <>
            ...
            >
                {/* {selected} - {selectedTrim} - {trimOption} */}
                {
                    ((selectedManufacturerName === "") || (selectedModelName === "")) && <><div className="bg-white p-3 border-b-2 border-blue-300">You have to select a model first</div></>
                }

                {
                    (trims.length === 0 && modelId != -1) && ((selectedManufacturerName != "") || (selectedModelName != "")) && <><div className="bg-white p-3 border-b-2 border-blue-300">No trim for selected model</div></>
                }
                
                
                { 
                    ((trims.length > 0) && (selectedManufacturerName != "") && selectedModelName != "") && <>
                        <div
                        ...
                        >
                            ...
                            } */}
                            {
                                selected
                                    ? selected?.length > 25
                                    ? selected?.substring(0, 25) + "..."
                                    : (selected === 'reset-trim') ? '- Select Trim -' : (selected === 're-reset') ? '- Select Trim -' : selected
                                    : placeholder 
                            }
                            {/* {
                                choosen  ?  choosen?.length > 25 ? choosen?.substring(0, 25) + "..." : (choosen === 'reset-trim') ?  (change === 'renew') ? choosen : '- Select Trim -' : choosen  : placeholder
                            } */}

...
export const UploadCourseModal = ({onClick, openCourseModal, data, token}: UploadCourseModalProps)  =>
{      
    const [loading, setIsLoading] = useState<boolean>(false)
    const [courseId] = useState<string>(data?.id.toString())
    ...
    const uploadCourseFile = async () =>
    {
        ...
        // return false
        await axios.post(`${BASE_URL}academic/course/upload-course-document`, fileDocument, {
           headers: {
                ...
                if(response?.status === 200)
                {
                   ...
                   setErrorMessage("Docment could not be uploaded")
                   setIsLoading(false)
                }
           }).catch(() => { 
